...
## Fix Integrazioni "in caricamento infinito" (2026-06)
- app/altro/integrazioni.tsx: prima, se `/integrations` falliva, `data` restava null e la pagina mostrava Loading per sempre. Ora: `load()` con setLoading(true)+finally; se loading→spinner, se `!data`→stato errore con pulsante "Riprova". Verificata su desktop: Meta/WhatsApp/AI ATTIVE.

## Desktop più largo + sezione "Contatti corso da chiamare" (2026-06)
- _layout.tsx: web frame maxWidth 480 → 1100 (vera larghezza desktop, niente più colonna stretta).
- (tabs)/_layout.tsx: barra inferiore web extra 16→22, paddingTop 8→10, iconLabelArea 52→56 (etichette mai tagliate).
- home_priorities: separa i contatti "corso" (servizio/campagna/inserzione/form_name contiene "corso") → nuova lista `corso_da_chiamare`; `da_fissare` ora contiene solo i trattamenti. priorities.tsx: nuova sezione "CONTATTI CORSO DA CHIAMARE".
- Verificato su desktop 1440: layout largo, sezione corso popolata, nulla tagliato. Fatto in una sola passata per contenere i crediti.
...
## Fix invio WhatsApp + Moduli raccolta dati + Foto messaggio iniziale (2026-06)
- **RISOLTO invio WhatsApp**: il token WA salvato era scaduto ("user logged out") E il phone_number_id era errato (`1785595714842859` inesistente). Corretto in db.integration_config: token = token permanente System User (scope whatsapp_business_messaging, non scade), phone_number_id = **1283595714842859** (+39 393 470 6525 "J'adore Mimì", GREEN). Verificato GET 200. Aggiunta `whatsapp_send_image`. Helper page-token per query leadgen_forms (serve Page token).
- **Risposta flusso Modulo Meta**: sì, all'arrivo del lead da modulo Supergirl crea contatto, apre conversazione con AI attiva, invia il primo messaggio via template approvato (`nuovo_lead_foto` APPROVED) e avvia i follow-up. Non resta solo in elenco.
- **Moduli raccolta dati (Lead Forms)**: `GET /api/meta/lead-forms` (elenca SG-), `POST /api/meta/lead-forms` (crea con FULL_NAME+PHONE, **niente email**, prefisso SG-, locale it_IT, privacy+follow_up = sito pagina, ACTIVE). Testato: creati "SG- Modulo Raccolta Dati" e "SG- Trattamenti". Campagna Modulo verificata e2e con lead_form_id.
- **Foto messaggio WhatsApp iniziale**: campo `welcome_image_url` per campagna, endpoint `POST /api/meta/upload-welcome-photo` (object storage) + `PATCH /api/meta/campaigns/{id}/welcome` (modificabile in qualsiasi momento). All'ingresso lead trattamento su WhatsApp, se la campagna ha la foto, viene inviata come immagine con didascalia (benvenuto) prima dei 2 pulsanti.
- **Frontend crea-campagna**: selettore/creatore Modulo (visibile con destinazione Modulo), uploader foto benvenuto (sezione WhatsApp, da tutte le sorgenti via expo-document-picker), oltre a testo annuncio + benvenuto + 2 domande già presenti. Tutto desktop, verificato con screenshot.


## Nuovo template lead-modulo + stop emoji (2026-06)
- Creato template `sg_lead_modulo_promo` (it, MARKETING) via Marketing API: HEADER IMAGE (foto dinamica del trattamento), BODY "Ciao cara, ho visto che hai richiesto informazioni per il trattamento {{1}}. In questo periodo puoi provarlo ancora a metà prezzo per i prossimi 10 giorni, fino a esaurimento posti." + 2 BUTTONS QUICK_REPLY "Quanto costa?" / "Dove vi trovate?". Immagine campione caricata via resumable upload (app_id) per l'header_handle. Stato: APPROVED.
- `whatsapp_send_template` ora sceglie il template PREFERITO (`template_name_preferred=sg_lead_modulo_promo`) se APPROVED (cache 10 min via `approved_template_names`), altrimenti fallback `nuovo_lead_foto`. Param corpo adattati: nuovo template = 1 var ({{1}}=servizio/trattamento); vecchio = 2 var (nome, servizio). Foto e nome trattamento si adattano alla campagna del lead. App mai bloccata durante l'approvazione. Aggiunto `template_name_preferred` a WA_ENV.
- STOP EMOJI definitivo: `strip_hearts` ora rimuove QUALSIASI emoji/emoticon (EMOJI_RE ampio) da tutte le risposte di Andrea. Prompt AI aggiornati ("VIETATO usare qualsiasi emoji o emoticon"). Rimosse le emoji hardcoded (😊) da greeting/message_options/esempi.
